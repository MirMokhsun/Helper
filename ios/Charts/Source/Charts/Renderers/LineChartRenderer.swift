...
open class LineChartRenderer: LineRadarRenderer
{
    ...
    open var predictionMarkupRenderer: PredictionMarkupRenderer?
    open var predictionMarkup: PredictionMarkup?
    
    public init(dataProvider: LineChartDataProvider, animator: Animator, viewPortHandler: ViewPortHandler, predictionMarkup: PredictionMarkup? = nil)
    {
        super.init(animator: animator, viewPortHandler: viewPortHandler)
        
        self.dataProvider = dataProvider
        self.predictionMarkupRenderer = PredictionMarkupRenderer(viewPortHandler: viewPortHandler, predictionMarkup: predictionMarkup ?? nil)
    ...
    @objc open func drawLinear(context: CGContext, dataSet: ILineChartDataSet)
    {
        ...
        let lastPoint = _lineSegments[_lineSegments.count-1];
        if((predictionMarkup) != nil){
            let openClosePrediction = getOpenClosePrediction(dataSet: dataSet)
            let stopLossTakeProfit = getStopLossTakeProfitYPosition(dataSet: dataSet)

            predictionMarkupRenderer?.drawPredictionMarkup(
                context: context,
                openClosePrediction: openClosePrediction,
                stopLossTakeProfit: stopLossTakeProfit,
                predictionPoint: CGPoint(x: lastPoint.x+50, y: lastPoint.y)
            )
        }
        context.restoreGState()
    ...
    func getOpenClosePrediction( dataSet: ILineChartDataSet) -> [CGPoint] {
        guard let dataProvider = dataProvider else { return [] }
        let trans = dataProvider.getTransformer(forAxis: dataSet.axisDependency)
        let valueToPixelMatrix = trans.valueToPixelMatrix
        let phaseY = animator.phaseY
        var openClosePrediction = [CGPoint](repeating: CGPoint(), count: 2)

        let openIndex =  predictionMarkupRenderer?.predictionMarkup?.openPredictionIndex ?? 0.0;
        let closeIndex =  predictionMarkupRenderer?.predictionMarkup?.closePredictionIndex ?? 0.0;
        openClosePrediction[0] =  CGPoint(x: CGFloat(openIndex), y: CGFloat(closeIndex * Float(phaseY)))
        openClosePrediction[1] =  CGPoint(x: CGFloat(closeIndex), y: CGFloat(openIndex * Float(phaseY)))
        openClosePrediction[0] = openClosePrediction[0].applying(valueToPixelMatrix)
        openClosePrediction[1] = openClosePrediction[1].applying(valueToPixelMatrix)

        return openClosePrediction;
    }
    
    func getStopLossTakeProfitYPosition( dataSet: ILineChartDataSet) -> [CGPoint] {
        guard let dataProvider = dataProvider else { return [] }
        let trans = dataProvider.getTransformer(forAxis: dataSet.axisDependency)
        let valueToPixelMatrix = trans.valueToPixelMatrix
        let phaseY = animator.phaseY
        var stopLossTakeProfit = [CGPoint](repeating: CGPoint(), count: 2)
        
        let stopLoss =  predictionMarkupRenderer?.predictionMarkup?.stopLoss ?? 0.0;
        let takeProfit =  predictionMarkupRenderer?.predictionMarkup?.takeProfit ?? 0.0;
        
        stopLossTakeProfit[0] =  CGPoint(x: CGFloat(stopLoss), y: CGFloat(takeProfit * Float(phaseY)))
        stopLossTakeProfit[1] =  CGPoint(x: CGFloat(takeProfit), y: CGFloat(stopLoss * Float(phaseY)))
        stopLossTakeProfit[0] = stopLossTakeProfit[0].applying(valueToPixelMatrix)
        stopLossTakeProfit[1] = stopLossTakeProfit[1].applying(valueToPixelMatrix)
